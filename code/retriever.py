...
# Top-K for retrieval — configurable via env
DEFAULT_TOP_K = int(os.getenv("RETRIEVAL_TOP_K", 6))
# Company boost multiplier — configurable via env
COMPANY_BOOST = float(os.getenv("COMPANY_BOOST", 1.5))
...
class CorpusRetriever:
    def __init__(
        ...
        # All params are overridable via environment variables
    ...
    def load_and_index(self, force_reindex: bool = False) -> None:
        ...
        debug_log(
            ...
            data={"model": self.model_name, "chunk_chars": self.chunk_chars},
        )
        ...
        self._embeddings = self.build_index(self._chunks, force=force_reindex)
    ...
    def chunk_documents(self, docs: Iterable[CorpusDoc]) -> list[Chunk]:
        """
        Word-boundary chunking with sliding window overlap.
        
        Splits on whole words to avoid cutting mid-token which can hurt embedding
        quality. Falls back to sentence boundaries when possible.
        """
        chunks: list[Chunk] = []
        chunk_id = 0

        # Estimate words per chunk (~5 chars per word average)
        ...
        for d in docs:
            ...
            # Try sentence-aware splitting first for short docs
            sentences = _re.split(r'(?<=[.!?])\s+', text)
            # Rebuild into word list but track sentence boundaries
            words = text.split()
            n_words = len(words)
            ...
            for start_word in range(0, n_words, step):
                ...
                # Trim trailing incomplete sentence if not at document end
                if end_word < n_words:
                    # Find last sentence boundary (ends with . ! ?)
    ...
    def build_index(self, chunks: list[Chunk], force: bool = False) -> np.ndarray:
        ...
        for d in self._docs:
            try:
                p = Path(d.filepath)
                corpus_fingerprint.append({"p": d.filepath, "m": p.stat().st_mtime_ns, "s": d.source_company})
            except Exception:
            ...
            try:
                ...
                if cached_meta == meta_key:
                    ...
                    if isinstance(emb, np.ndarray) and emb.shape[0] == len(chunks):
                        ...
                        debug_log(run_id=os.getenv("DEBUG_RUN_ID", "run"), hypothesis_id="H1",
                                  location="retriever.py:build_index", message="Cache hit", data={"rows": int(emb.shape[0])})
                        return emb
            except Exception:
                pass

        if force:
            print("[retriever] Force reindex — rebuilding from scratch.")

        print(f"[retriever] Building embedding index over {len(chunks)} chunks…")

        if SentenceTransformer is None:
            raise RuntimeError("sentence-transformers is required. Run: pip install -r requirements.txt")
